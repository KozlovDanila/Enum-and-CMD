...
public class CMD {
    ...
    private static double getResult(String[] args) {
        ...
        Operation n = Operation.valueOf(operation);
        return n.calculate(leftOperand, rightOperand);
    }
    ...
    private static int getOperand(String[] args, String operand) {
        ...
        if (!isNextNumber(args, indexOfRight + 1)) {
            throw new IllegalArgumentException();
        }
        return Integer.parseInt(args[indexOfRight + 1]);
    }

    private static boolean isNextNumber(String[] args, int i) {
        String str = args[i];
